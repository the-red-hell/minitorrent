use bencode::{BencodeParser, Result, Error};

#[derive(Debug, PartialEq)]
pub struct MyMetaInfo<'a> {
    pub announce: &'a str,
    pub length: i64,
    pub name: &'a str,
}
...
mod tests {
    ...
    fn test_parse_basic_metainfo() {
        ...
        let meta = MyMetaInfo::parse(input).unwrap();
    ...
    fn test_parse_metainfo_with_list_field() {
        let input = b"d8:announce15:http://test.com10:extra-listli1ei2ei3ee6:lengthi1234e4:name8:testfilee";
        let meta = MyMetaInfo::parse(input).unwrap();
    ...
    fn test_parse_metainfo_missing_announce() {
        ...
        let result = MyMetaInfo::parse(input);
    ...
    fn test_parse_metainfo_special_chars_in_strings() {
        let input = b"d8:announce23:http://test.com:8080/tr6:lengthi1234e4:name18:test file-v1.0.txte";
        let meta = MyMetaInfo::parse(input).unwrap();

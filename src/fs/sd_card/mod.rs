...
pub struct FileSystemInitializer<'a, SCK, MISO, MOSI, CS>
where
...
impl<'a, SCK, MISO, MOSI, CS> FileSystemInitializer<'a, SCK, MISO, MOSI, CS>
where
    SCK: gpio::interconnect::PeripheralOutput<'a>,
    MISO: gpio::interconnect::PeripheralInput<'a>,
    MOSI: gpio::interconnect::PeripheralOutput<'a>,
...
impl<'a> SdCard<'a> {
    pub fn init<'b, SPI, SCK, MISO, MOSI, CS>(
        initializer: FileSystemInitializer<'b, SCK, MISO, MOSI, CS>,
        spi: SPI,
        ...
        CS: gpio::OutputPin + 'b,
    {
        ...
        let num_bytes = loop {
            match sdcard.num_bytes() {
                ...
                Err(e) => {
                    defmt::warn!("failed to initialize card with error {:?}", e);
                    delay.delay_millis(500);
                }
            }
        };
        defmt::info!("Card size is {:?} MB", num_bytes);
...
impl<'a> From<SdCard<'a>> for VolumeMgr<'a> {
    fn from(value: SdCard<'a>) -> Self {
        ...
        defmt::info!("has opened handles: {}", volume_mgr.has_open_handles());
